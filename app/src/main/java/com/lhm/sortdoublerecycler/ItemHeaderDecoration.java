...
public class ItemHeaderDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDrawOver(Canvas canvas, final RecyclerView parent, RecyclerView.State state) {
        ...
        int type = mDatas.get(pos).getIsTitle();
        ...
        if (!TextUtils.equals(mDatas.get(pos).getTag(), mDatas.get(pos + 1).getTag())
            ...
            if (type == 1) { //获取item item类型为title 时平移
                //body 才平移
...
//        ImageView img = (ImageView) topTitleView.findViewById(R.id.img);
        tvTitle.setText(mDatas.get(pos).getTitleName());
//        if (TextUtils.isEmpty(mDatas.get(pos).getImgsrc())){
//            img.setImageResource(R.drawable.dk_data_clean);
//        }
        //绘制title开始
